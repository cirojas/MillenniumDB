...
private:
    ...
    // setted when visiting a fixed node or edge
    Id last_object = ObjectId::get_null();

    // used to remember a node when traversing linear patterns
    ...
    bool inside_node; // used to know if inside a node or edge when visiting a property
