...
public:
    ...
    OpConstruct(std::vector<OpTriple>&& triples, std::unique_ptr<Op> op, uint64_t offset, uint64_t limit) :
    ...
    std::unique_ptr<Op> clone() const override
    {
        ...
        return std::make_unique<OpConstruct>(std::move(new_triples), op->clone(), offset, limit);
    }
