...
class OpNotExists : public Op {
public:
    std::unique_ptr<Op> lhs;
    std::unique_ptr<Op> rhs;

    OpNotExists(std::unique_ptr<Op> lhs, std::unique_ptr<Op> rhs) :
        lhs(std::move(lhs)),
        rhs(std::move(rhs))
    { }
    ...
    {
        return std::make_unique<OpNotExists>(lhs->clone(), rhs->clone());
    }
