...
namespace SPARQL {
class BindingExprStrEnds : public BindingExpr {
public:
    std::unique_ptr<BindingExpr> lhs;
    std::unique_ptr<BindingExpr> rhs;

    BindingExprStrEnds(std::unique_ptr<BindingExpr> lhs, std::unique_ptr<BindingExpr> rhs) :
        lhs(std::move(lhs)),
        rhs(std::move(rhs))
    { }
    ...
    ObjectId eval(const Binding& binding) override
    {
        ...
        auto suffix = lhs_str.substr(lhs_size - rhs_size);
        return Conversions::pack_bool(suffix == rhs_str);
    }
