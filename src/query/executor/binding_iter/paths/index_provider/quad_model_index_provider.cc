...
std::unique_ptr<EdgeIter> QuadModelIndexProvider::get_btree_iter(uint64_t label_id, bool inverse)
{
    // B+Tree settings
    Record<4> min_ids = { label_id, 0, 0, 0 };
    Record<4> max_ids = { label_id, UINT64_MAX, UINT64_MAX, UINT64_MAX };

    // Get iter from correct B+Tree
    if (inverse) {
        return std::make_unique<BTreeIndexIterator<4>>(
            quad_model.label_to_from_edge->get_range(interruption_requested, min_ids, max_ids),
            1,
            2,
            0
        );
        ...
        return std::make_unique<BTreeIndexIterator<4>>(
            quad_model.label_from_to_edge->get_range(interruption_requested, min_ids, max_ids),
            1,
            2,
            0
        );
...
std::unique_ptr<EdgeIter>
    QuadModelIndexProvider::get_btree_iter(uint64_t label_id, bool inverse, uint64_t node_id)
{
    // B+Tree settings
    Record<4> min_ids = { label_id, node_id, 0, 0 };
    Record<4> max_ids = { label_id, node_id, UINT64_MAX, UINT64_MAX };
...
std::unique_ptr<EdgeIter> QuadModelIndexProvider::get_outgoing(uint64_t node_id)
{
    ...
    return std::make_unique<BTreeIndexIterator<4>>(
        quad_model.from_to_label_edge->get_range(interruption_requested, min_ids, max_ids),
        0,
        1,
        2
    );
...
std::unique_ptr<EdgeIter> QuadModelIndexProvider::get_incoming(uint64_t node_id)
{
    ...
    Record<4> max_ids = { node_id, UINT64_MAX, UINT64_MAX, UINT64_MAX };
    return std::make_unique<BTreeIndexIterator<4>>(
        quad_model.to_label_from_edge->get_range(interruption_requested, min_ids, max_ids),
        0,
        2,
        1
    );
