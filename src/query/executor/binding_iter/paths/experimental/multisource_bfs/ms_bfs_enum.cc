...
next_begin:
    while (ready_solutions.size() > 0) {
        ...
        if constexpr (MULTIPLE_FINAL) {
            EndpointSolution s(solution.start_index, solution.state->node_id);
            if (!reached_final.insert(s).second) {
                continue;
            }
        }
        solution.state->start_node = start_batch[solution.start_index];
        solution.state->start_node_idx = solution.start_index;

        auto path_id = path_manager.set_path(solution.state, path_var);
        parent_binding->add(path_var, path_id);
        parent_binding->add(start, solution.state->start_node);
        parent_binding->add(end, solution.state->node_id);
        return true;
    }
...
bool BFSMultiSource<MULTIPLE_FINAL>::expand_neighbors(const MSSearchState& current_state)
{
    ...
    while (current_transition < automaton.from_to_connections[current_state.automaton_state].size()) {
        ...
        while (iter->next()) {
            ...
            MSSearchState next_state(transition.to, reached_node);
