...
public:
    ...
    std::unique_ptr<BPlusTree<4>> from_to_label_edge;
    std::unique_ptr<BPlusTree<4>> to_label_from_edge;
    std::unique_ptr<BPlusTree<4>> label_from_to_edge;
    std::unique_ptr<BPlusTree<4>> label_to_from_edge;
    std::unique_ptr<BPlusTree<4>> edge_from_to_label;

    // special cases
    std::unique_ptr<BPlusTree<3>> equal_from_to; // (from=to, label, edge)
    std::unique_ptr<BPlusTree<3>> equal_from_to_inv; // (label, from=to, edge)
